...
public class SoundManager : SingletonWithMonoBehaviour<SoundManager> {
    ...
    private void Awake() {
        ...
        assetLoader.LoadAssetsAsync<AudioClip>(gameSEPath, (x) => {
            _gameSEAudioClipList = x.Result as List<AudioClip>;
        });

        string bgmPath = "BGM";
        assetLoader.LoadAssetsAsync<AudioClip>(bgmPath, (x) => {
            _bgmAudioClipList = x.Result as List<AudioClip>;
        });
